...
class TaskBatchSampler(Sampler):
    ...
    Similar to how LIBERO naturally groups trials by task, this sampler ensures
    WorldGym batches have the same instruction.
    """
...
class WORLDGYM_Dataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        if "worldgym" in self.task_suite_name:
            ...
            # Assign task_id based on instruction group (like LIBERO does with tasks)
            for task_id, instruction in enumerate(sorted_instructions):
                trials_for_instruction = instruction_groups[instruction]

                # Handle num_trials_per_task: repeat trials if needed (like Robotwin does)
                if self.train_val == "train":
